...
function loadConfig() {
  ...
  if (!fs.existsSync(configPath)) {
    console.error('❌ Configuration not found: .figma/export-config.json');
    process.exit(1);
  }

  if (!fs.existsSync(mappingPath)) {
    console.error('❌ Token mapping not found: .figma/token-mapping.json');
    process.exit(1);
  }
...
function fetchFigmaTokens(fileId, accessToken) {
  ...
  console.log('⚠️  Note: This requires Figma API integration via MCP server\n');
...
function main() {
  ...
  if (!accessToken) {
    console.log('⚠️  FIGMA_PERSONAL_ACCESS_TOKEN not set');
    console.log('   Set environment variable or configure in Claude Desktop config\n');
    console.log('📝 Using mock tokens for demonstration...\n');
  }

  const tokens = fetchFigmaTokens(figmaFileId, accessToken);

  console.log('🔄 Mapping tokens to SCSS...');
  ...
  console.log(`📄 Output: ${config.exports.designTokens.outputFile}\n`);
